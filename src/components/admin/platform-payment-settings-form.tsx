...
export function PlatformPaymentSettingsForm({ settings }: { settings: Settings }) {
  const [qrUrl, setQrUrl] = useState(settings?.paymentQrImageUrl ?? "")
  ...
  async function save() {
    setSaving(true)
    const result = await updatePlatformPaymentSettings({
      paymentQrImageUrl: qrUrl,
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          ...
          <ImageUpload onUploaded={setQrUrl} label={qrUrl ? "Replace QR image" : "Upload QR image"} />
