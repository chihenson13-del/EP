/** Escapes text for safe inclusion in HTML — names/event titles are user-controlled and must never be treated as markup. */
...
/** Plain-text organizer messages → safe HTML (escaped, newlines preserved). */
...
<html>
  <body style="margin:0;padding:0;background:#fff9f2;font-family:'Segoe UI',Helvetica,Arial,sans-serif;">
    <span style="display:none;font-size:1px;color:#fff9f2;">${escapeHtml(preheader)}</span>
...
function button(url: string, label: string): string {
  return `<a href="${escapeHtml(url)}" style="display:inline-block;margin-top:20px;padding:12px 24px;background:#6f5a86;color:#fff;text-decoration:none;border-radius:999px;font-weight:600;">${escapeHtml(label)}</a>`
}

export function verifyEmailTemplate(name: string, url: string) {
  return shell(
    "Verify your email to activate your account",
    `<p>Hi ${escapeHtml(name)},</p><p>Thanks for creating an Events Partner account. Please verify your email address to get started.</p>${button(url, "Verify email")}<p style="margin-top:20px;color:#756d78;font-size:13px;">This link expires in 24 hours.</p>`
  )
}
...
export function resetPasswordTemplate(name: string, url: string) {
  return shell(
    ...
    `<p>Hi ${escapeHtml(name)},</p><p>We received a request to reset your password. If this was you, click below to choose a new one.</p>${button(url, "Reset password")}<p style="margin-top:20px;color:#756d78;font-size:13px;">This link expires in 1 hour. If you didn't request this, you can ignore this email.</p>`
  )
}
...
export function invitationTemplate(opts: {
  ...
  return shell(
    ...
    `<p>Hi ${escapeHtml(opts.guestName)},</p>
     <p>${opts.message ? messageBodyToHtml(opts.message) : `${escapeHtml(opts.hostName ?? "You")} would love for you to join <strong>${escapeHtml(opts.eventName)}</strong>.`}</p>
     ${opts.dateLabel ? `<p><strong>When:</strong> ${escapeHtml(opts.dateLabel)}</p>` : ""}
     ${opts.venueLabel ? `<p><strong>Where:</strong> ${escapeHtml(opts.venueLabel)}</p>` : ""}
...
export function reminderTemplate(opts: { guestName: string; eventName: string; dateLabel?: string; rsvpUrl: string }) {
  return shell(
    ...
    `<p>Hi ${escapeHtml(opts.guestName)},</p><p>Just a friendly reminder about <strong>${escapeHtml(opts.eventName)}</strong>${opts.dateLabel ? ` on ${escapeHtml(opts.dateLabel)}` : ""}. We'd love to know if you can make it.</p>${button(opts.rsvpUrl, "RSVP now")}`
  )
}
...
export function paymentStatusTemplate(name: string, status: "APPROVED" | "PENDING" | "REJECTED", planName: string) {
  ...
  return shell(`Payment ${status.toLowerCase()}`, `<p>Hi ${escapeHtml(name)},</p><p>${escapeHtml(copy)}</p>`)
}
