...
export async function getGalleryForEvent(eventId: string, opts: { includeHidden?: boolean } = {}): Promise<GalleryItem[]> {
  const rows = await db.$queryRaw<{ id: string; url: string | null; caption: string | null; hidden: boolean }[]>`
    SELECT id,
  ...
  return rows.map((r) => ({ id: r.id, url: r.url ?? `/api/media/gallery/${r.id}`, caption: r.caption, hidden: r.hidden }))
}
